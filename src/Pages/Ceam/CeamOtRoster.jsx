...
import { SlButton, SlDialog, SlSelect, SlInput, SlOption } from '@shoelace-style/shoelace/dist/react/index.js';
			...
			<SlDialog label="Update/Add Single Reward" open={openUpSignleOt} onSlRequestClose={() => setOpenUpSignleOt(false)}>
				<div className="update-ot-single-main">
					...
					<SlSelect
						label="Reward Type"
						onSlChange={(e) => {
							setSignleOtUpData({ ...signleOtUpData, ot_type: e.target.value.split('_').join(' ') });
						}}>
						<SlOption value="">Remove</SlOption>
						{otList?.map((item, i) => {
							return (
								<SlOption key={`${i}ot`} value={item.shift_character.split(' ').join('_')}>
									{item.shift_character}
								</SlOption>
							);
						})}
					...
					<SlSelect
						label="Plant"
						onSlChange={(e) => {
							setSignleOtUpData({ ...signleOtUpData, plant: e.target.value.split('_').join(' ') });
							getDivision(e.target.value.split('_').join(' '));
						}}>
						{plantList?.map((item, i) => {
							return (
					...
					<SlSelect
						label="Division"
						onSlChange={(e) => {
							setSignleOtUpData({ ...signleOtUpData, division: e.target.value.split('_').join(' ') });
						}}>
						{divisionList?.map((item, i) => {
							return (
